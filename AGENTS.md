...
| Cards / starter / rewards | `src/data/cards.ts` → [docs/CONTENT.md](docs/CONTENT.md) |
| Monsters / intents / roles | `src/data/enemies.ts` |
| Multi-enemy packs | `src/data/encounters.ts` |
| Map topology / room kinds | `src/data/map.ts` → [docs/MAP.md](docs/MAP.md) |
| Phrases / packs | `src/data/phrases.ts`, `src/game/phraseSettings.ts` |
| Combat rules | `src/game/battle/*` via `src/game/combat.ts` → [docs/COMBAT.md](docs/COMBAT.md) |
| Cast / spell check | `src/game/castCheck.ts`, `src/ui/castView.ts` |
...
| Styles / 1080p stages | `src/styles/main.css` |

## Architecture in one breath

Single mutable `RunState` in `main.ts`. `render()` rebuilds `#app` from `state.screen`. Views use `ui/runtime` (`run()`, `render()`, session flags). Combat implementation is modular under `game/battle/`; facade at `game/combat.ts`. Debug panel mounts on **`document.body`** (survives `#app` wipe).
...
[docs/INDEX.md](docs/INDEX.md) · Architecture · Combat · Map · Content · Balance · Debug · [Roadmap](docs/ROADMAP.md)
